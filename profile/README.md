...
1. **感知上下文**：从任务目标、约束、数据和历史中提取真正需要解决的问题。
2. **动态组合**：按当前任务选择成员、模型、工具、数据和算力，形成临时的协作单元。
3. **边界内试错**：在宿主策略、最小权限和隔离执行的保护下快速验证假设。
4. **反馈回流**：把有效的判断、过程和结果交给评估与复盘，为下一次结晶准备素材。

流体智力负责在未知中发现新解，晶体智力负责把有效的新解保存、验证并放大。治理是这两种智力能够安全循环的边界机制。

### 晶体智力 · Crystal Intelligence

**晶体智力**是团队在长期协作中形成的结构化、可验证、可复用的认知资产。它是流体智力有效成果的沉淀，像晶体从成核点持续生长：一个人的判断带着上下文被记录，经过评估后变成 Skill、Tool、知识、记忆或工作流，再被更多成员和 Agent 调用。
